    ...
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: 24, gap: 20 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, justifyContent: 'center' }}>
          {[1, 2, 3, 4, 5].map(n => (
            <TouchableOpacity key={n} onPress={() => setNota(n)}>
              <Ionicons name={n <= nota ? 'star' : 'star-outline'} size={40} color={n <= nota ? colors.warning : colors.border} />
            </TouchableOpacity>
          ))}
        </View>
        {nota > 0 && (
      ...
      <View style={{ backgroundColor: colors.surface, borderRadius: 20, padding: 20, gap: 10, borderWidth: 1, borderColor: colors.border }}>
        <Text style={{ fontSize: 15, fontWeight: '800', color: colors.text }}>
          Comentário <Text style={{ color: colors.textMuted, fontWeight: '500' }}>(opcional)</Text>
        </Text>
        <TextInput
          style={{ backgroundColor: colors.surface2, borderRadius: 14, padding: 14, fontSize: 14, color: colors.text, borderWidth: 1, borderColor: colors.border, minHeight: 100, textAlignVertical: 'top' }}
